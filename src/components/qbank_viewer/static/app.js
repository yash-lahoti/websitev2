...
(function () {
  ...
  function recordStudyAttempt(qid, wasCorrect) {
    const track = $("set-track-attempts");
    if (!track || !track.checked) return;
    const map = loadStudyMap();
    const prev = map[qid] || { attempts: 0, correct: 0, lastAt: 0 };
    map[qid] = {
  ...
  function manifestUrl() {
    return "/api/generated/manifest";
  }

  function invalidateUrl() {
    return "/api/generated/manifest/invalidate";
  }
  ...
  async function fetchQuestion(qid) {
    const url = `/api/generated/question/${encodeURIComponent(qid)}`;
    const r = await fetch(url);
    if (!r.ok) throw new Error(await r.text());
    return r.json();
  }

  async function patchOutcome(qid, outcome, letter) {
    const body = JSON.stringify({
      outcome: outcome.toLowerCase(),
      selected_letter: letter ?? null,
    });
    const r = await fetch(
      `/api/generated/question/${encodeURIComponent(qid)}/quiz`,

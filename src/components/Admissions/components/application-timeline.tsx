...
export function ApplicationTimeline() {
  ...
  const phasesToShow = phases.filter((p) => visiblePhases.includes(p.id));
  const gridCols = phasesToShow.length;
    ...
    <section id="timeline" className="py-24 bg-background">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-6 md:mb-10">
          <p className="text-primary font-medium mb-2 text-sm sm:text-base">Plan Your Journey</p>
        ...
        {/* Sticky controls: duration + categories - stay in view while scrolling */}
        <div className="sticky top-14 z-10 bg-background/95 backdrop-blur-sm py-3 -mx-4 px-4 sm:-mx-6 sm:px-6 rounded-lg border-b border-border/50 mb-4 md:mb-6">
          {/* Duration Toggle */}
          <div className="flex justify-center mb-3 md:mb-4">
            <div className="inline-flex bg-secondary/50 rounded-lg sm:rounded-xl p-0.5 sm:p-1 border border-border w-full sm:w-auto max-w-sm sm:max-w-none">
              {durations.map((duration) => (
                <button
                  ...
                  className={cn(
                    "relative flex-1 sm:flex-none px-2 sm:px-4 md:px-5 py-2 sm:py-2.5 rounded-md sm:rounded-lg text-xs sm:text-sm font-medium transition-all duration-300 min-w-0",
                ...
                >
                  <span className="font-bold block truncate">{duration.label}</span>
                  <span className="text-[10px] sm:text-xs opacity-80 hidden sm:block truncate">
                    {duration.subtitle}
                  </span>
                </button>
              ))}
          ...
          {selectedDuration === "6months" && (
            <div className="flex items-center justify-center gap-2 mb-3 p-2.5 sm:p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg sm:rounded-xl max-w-xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
            {Object.entries(categories).map(([key, cat]) => (
              <div key={key} className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
                <div className={cn("w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-sm shrink-0", cat.bg)} />
        ...
        {/* Main Timeline - only visible phases; stack on mobile, grid on desktop */}
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          {/* Mobile: stacked phases (one column) */}
                    ...
                    {phaseCards.map((card) => {
                      ...
                      return (
                        <div
                          key={card.id}
                          className={cn(
                            "p-3 rounded-lg border transition-all touch-manipulation",
                            cn(cat.bgLight, cat.border, "active:scale-[0.99]")
                          )}
                        >
          ...
          {/* Desktop: phase headers row + cards grid */}
          <div className="hidden md:block">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-center gap-2 flex-wrap">
                      ...
                      />
                      <span className="font-semibold text-foreground text-sm">{phase.title}</span>
                      <span className="text-xs text-muted-foreground">({phase.subtitle})</span>
                    </div>
                  </div>
                );
            ...
            >
              {phasesToShow.map((phase) => {
                const phaseCards = timelineCards.filter((c) => c.phase === phase.id);
                return (
                  <div key={phase.id} className="p-3 border-r last:border-r-0 border-border min-w-0">
                      ...
                      {phaseCards.map((card) => {
                        ...
                        return (
                          <div
                            key={card.id}
                            className={cn(
                              "p-3 rounded-lg border transition-all duration-300 min-w-0",
